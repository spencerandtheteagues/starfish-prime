...
/**
 * Helper function to send notifications (can be called from other functions)
 */
export async function sendNotificationHelper(data: {
  seniorId: string;
  caregiverId: string;
  seniorName: string;
  flags: RiskFlag[];
  ...
  try {
    ...
    console.log(`Notification sent to ${results.successCount} devices`);
  } catch (error) {
    console.error('Error sending risk notification:', error);
