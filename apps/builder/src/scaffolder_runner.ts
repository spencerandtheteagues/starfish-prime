...
export async function scaffold_app(runDir:string, plan:Plan){
  ...
  await mk('package.json', JSON.stringify({
    name:'generated-app',
    private:true,
    version:'1.0.0',
    description:'Generated full-stack application with AI builder',
    workspaces:['apps/*','prisma'],
    engines:{node:'>=20 <21'},
    ...
    devDependencies:{ vitest:'2.1.2', vite:'5.4.8', '@playwright/test':'1.47.2', typescript:'5.6.3', tsx:'4.19.2', '@types/node':'20.12.14' },
    license: 'MIT',
    author: 'AI Builder'
  },null,2));
  await mk('.gitignore','node_modules\n.next\ndist\n.env*\n');
  await mk('README.md', `# Generated App

This is a full-stack application generated by the Personal AI App Builder.

## Features

- **API**: Fastify-based REST API with Prisma ORM
...
${planEntities.map(e => `mem['${e.name}']=makeStore<any>();`).join('')}
...
`);
  ...
  for (const ent of planEntities){
    const lower=ent.name.toLowerCase(); const inputs = (ent.fields||[]).filter((f:any)=>f.name!=='id').map((f:any)=>f.name).map((n:string)=>`<div><label>${n}: <input name="${n}" /></label></div>`).join('\n') || '<div>No fields</div>';
    await mk(`apps/web/app/${lower}.tsx`, `'use client';
...
jobs:
  build-test:
    ...
    steps:
      ...
      - name: GitHub Spec Kit validation
        run: |
          uvx --from git+https://github.com/github/spec-kit.git specify validate . --output text || true
      - name: Install deps
